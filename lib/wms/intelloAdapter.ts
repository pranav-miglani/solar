...
export class IntelloAdapter extends BaseWmsAdapter {
  ...
  /**
   * Fetch token directly from Intello API
  ...
  protected async fetchTokenFromApi(): Promise<{
    ...
    if (!response.ok) {
      ...
      throw new Error(`Intello authentication failed: ${response.status} ${responseText}`)
    }
  ...
  async listSites(): Promise<WmsSite[]> {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Failed to fetch Intello sites: ${response.status} ${responseText}`)
    }
  ...
  async getInsolationData(
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Failed to fetch Intello insolation data: ${response.status} ${responseText}`
      )
    }
