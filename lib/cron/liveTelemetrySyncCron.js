...
function startLiveTelemetrySyncCron() {
    ...
    return MDC.runAsync(
      ...
      async () => {
        try {
          ...
          logger.info(`🕐 Live telemetry sync cron triggered at ${istHour}:${istMin.toString().padStart(2, '0')} IST`)
  ...
  logger.info('✅ Live telemetry sync cron job initialized - running every 15 minutes')
}
