...
function startLiveTelemetrySyncCron() {
  ...
  // Try to load MDC and logger, fallback to console if not available
  let MDC, logger
  try {
    MDC = require('../context/mdc').default
    logger = require('../context/logger').logger
  } catch (error) {
    // Fallback to console if TypeScript modules aren't available
    console.warn('[LiveTelemetrySyncCron] MDC/Logger not available, using console fallback:', error.message)
    MDC = {
      run: (context, fn) => fn(),
