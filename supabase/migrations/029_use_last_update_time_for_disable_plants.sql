-- ============================================
-- USE last_update_time FOR DISABLE PLANTS LOGIC
-- ============================================
-- This migration updates the disable_inactive_plants function
-- to use last_update_time (vendor's last data update) instead of
-- last_refreshed_at (our last sync time) for determining inactivity.
...
COMMENT ON FUNCTION disable_inactive_plants() IS 'Checks for plants that haven''t received vendor updates (last_update_time) in 3+ days, marks them as inactive (is_active = false), and copies their data to disabled_plants table. Returns count and IDs of disabled plants.';

-- Drop the old helper function first (to change parameter name)
DROP FUNCTION IF EXISTS should_disable_plant(TIMESTAMPTZ, TIMESTAMPTZ);

-- Create the helper function with updated parameter name to use last_update_time
CREATE FUNCTION should_disable_plant(plant_last_update_time TIMESTAMPTZ, plant_created_at TIMESTAMPTZ)
RETURNS BOOLEAN
LANGUAGE plpgsql
IMMUTABLE
AS $$
...
-- Update comment for days_since_refresh column (keeping name for backward compatibility)
